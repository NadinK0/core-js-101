...
function isTriangle(a, b, c) {
  ...
  const arr = [a, b, c];
  arr.sort((i, j) => i - j);
  if ((arr[0] ** 2 + arr[1] ** 2 === arr[2] ** 2) || (a === b && a === c)) {
    res = true;
  } else {
    res = false;
  }
...
function findFirstSingleChar(str) {
  let letter = null;
  const arr = Array.from(str).filter((e) => e !== ' ');
  for (let i = 0; i < arr.length; i += 1) {
    let newArr = arr.slice(0);
    newArr.splice(i, i + 1);
    newArr = newArr.filter((e) => e === arr[i]);
    if (newArr.length === 0) {
      letter = arr[i];
      return letter;
    }
...
function isCreditCardNumber(ccn) {
  const arr = Array.from(String(ccn));
  const arrCN = arr.map((i) => Number(i));
  let b;
  let sum = 0;
  const parity = (arrCN.length) % 2;
  for (let i = 0; i < arrCN.length; i += 1) {
    let digit = arrCN[i];
    if (i % 2 === parity) {
      digit *= 2;
      if (digit > 9) {
...
function getDigitalRoot(num) {
  let numb = num;
  function getSum(n) {
    const arr = Array.from(String(n)).map((i) => Number(i));
    const sum = arr.reduce((acc, el) => acc + el);
    return sum;
  }
  while (String(numb).length !== 1) {
    numb = getSum(numb);
  }
  return numb;
}
  ...
  for (let i = 0; i < arr.length - 1; i += 1) {
    for (let j = 0; j < arr[i].length; j += 1) {
      if (arr[i][j] === arr[i + 1][j]) {
        ...
        arr[i + 1] = newArr.slice('');
      }
    }
    arr[i + 1] = newArr.slice('');
    newArr.length = 0;
  }
  let ind = arr[arr.length - 1].indexOf('/');
  while (ind !== -1) {
    newArr.push(ind);
    ind = arr[arr.length - 1].indexOf('/', ind + 1);
  }
  return (arr[arr.length - 1].slice(0, newArr[newArr.length - 1] + 1).join(''));
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    for (let j = 0; j < position[i].length; j += 1) {
      if (position[i][j] === '0' && i + j === position.length - 1) {
        n += 1;
      } else if (position[i][j] === 'X' && i + j === position.length - 1) {
        k += 1;
      }
    }
    if (k === 3) {
